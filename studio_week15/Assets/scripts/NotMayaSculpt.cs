...
public class NotMayaSculpt : MonoBehaviour {
	...
	void Update () {
		...
		if ( Physics.Raycast(ray, out rayHit, 1000f) ) {
			...
			if ( Input.GetMouseButton(0) ) {
				...
				mf = rayHit.collider.GetComponent<MeshFilter>();
				...
				for( int i=0; i<vertices.Length; i++ ) {
					...
					if( distanceFromClickPoint < brushSize ) { // ... but only if the vertex is close enough to where we clicked
						vertices[i] += clickNormal * (distanceFromClickPoint / brushSize) * Time.deltaTime * 0.1f;
					}
				}
